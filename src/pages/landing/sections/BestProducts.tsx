...
const BestProducts = ({ showcase }: Props) => {
  ...
  return (
    <div className="p-4 bg-color-for-layer-sec pb-0 lg:pb-4 flex flex-col relative mt-4  lg:mb-4 lg:mt-0 rounded-2xl">
      <div className="flex-wrap items-center gap-2 flex">
        <div className="first-text-color-svg">
          ...
          >
            <path
              d="M8 21H16M12 21V17M12 17C9.23858 17 7 14.7614 7 12V6M12 17C14.7614 17 17 14.7614 17 12V6M17 6C17 4.89543 16.1046 4 15 4H9C7.89543 4 7 4.89543 7 6M17 6H18.5C19.8807 6 21 7.11929 21 8.5C21 9.88071 19.8807 11 18.5 11H17M7 6H5.5C4.11929 6 3 7.11929 3 8.5C3 9.88071 4.11929 11 5.5 11H7"
              stroke="currentColor"
              strokeWidth="1.5"
              strokeLinecap="round"
        ...
        {showcase?.items.slice(0, 3).map((item) => {
          return (
            <Link
              ...
              className="bg-color-for-layer-on-body rounded-xl relative flex  flex-wrap mt-3 p-3"
              key={item.product.id}
            >
              <span
                className="absolute right-1 flex flex-col items-end top-1 after:absolute after:left-full pr-1 after:h-full after:w-0.5 after:rounded-4xl
                after:rounded-tr-none after:rounded-br-none after:bg-secound  "
              >
                <span className="text-xs font-light  inline-block w-full  text-start text-secound">
                  {item.product.discount && <h3>{item.product.discount}%</h3>}
                </span>
              </span>
              <div className="flex items-center justify-between w-full">
                <div className="w-11/48 p-2 bg-color-for-layer-sec rounded-2xl">
                  <img
                ...
                <h2
                  className="font-s-light text-sm first-text-color w-16/48"
                  title={item.product.name}
                >
                  {item.product.name.split(' ').length > 5
                    ? item.product.name.split(' ').slice(0, 5).join(' ') + '...'
                    : item.product.name}
                </h2>
                <div className="flex flex-col   w-16/48  justify-between">
      ...
      <div className=" relative lg:absolute bottom-0 justify-center mt-4 lg:-bottom-8 rounded-md left-1/2 -translate-x-1/2 px-2 py-2 bg-first flex text-center">
        <button className="flex text-white text-center justify-center items-center">
          <Link to={`/products`} className="text-sm flex">
