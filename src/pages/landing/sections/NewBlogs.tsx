  ...
  return (
    <div className="sm:container mt-8 lg:mt-16 mx-auto px-4 relative">
      ...
      <div className="flex flex-wrap justify-between mt-5">
        <div className="lg:w-60/96 xl:w-45/96 w-full bg-color-for-layer-on-body p-8 rounded-2xl ">
          {blogs?.length && (
        ...
        <div className="lg:w-60/96 xl:w-50/96 w-full flex flex-col justify-between mt-3 lg:mt-0">
          {blogs?.length! > 0 &&
            blogs?.map((blog) => (
              <Link
                key={blog.id}
                to={`/blogs/${blog?.slug}`}
                className="w-full bg-color-for-layer-on-body rounded-2xl lg:first:mt-0 mt-3 p-8 flex  flex-row justify-between gap-3"
              >
                <div className=" h-24 w-34/96 md:w-16/96">
                  <img
                    className="w-full rounded-xl  sm:h-full object-cover object-right"
                    src={blog?.image}
                    alt={blog?.title || t('mainpage.blogs.imageAlt')}
                  />
                </div>
                <div className=" flex flex-col justify-between  w-60/96 md:w-78/96">
                  ...
                  <div className="flex justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm first-text-color-for-paragraph">{blog?.author}</span>
                    </div>
                    <div className="flex items-center ">
                      <span className="text-sm first-text-color-for-paragraph">
                        {blog?.readTime}
