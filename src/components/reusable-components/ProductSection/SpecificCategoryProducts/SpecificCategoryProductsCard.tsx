...
export const SpecificCategoryProductsCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div
      className={`h-100 cursor-pointer p-4 rounded-2xl flex flex-col justify-between transition-colors duration-300 ${
        isHovered ? 'bg-first text-white' : 'bg-color-for-layer-on-body first-text-color '
      }`}
          ...
          {isRTL ? (
            <div className="flex flex-col items-start text-right">
              <PriceDisplay
                ...
                languageCode="fa"
                variant="primary"
                className="font-s-sbold text-lg"
              />
              {product.originalPrice && (
                <span className=" first-text-color-for-paragraph relative">
                  <PriceDisplay
                    currencyMode="none"
