...
export async function getUserAddresses(langCode?: string): Promise<UserAddress[]> {
  ...
  const response = (await apiRequest(`/api/ui/checkout/addresses?${queryParams.toString()}`, {
    method: 'GET',
  })) as UserAddress[];
...
export async function getProvinces(langCode?: string): Promise<Province[]> {
  ...
  const response = (await apiRequest(`/api/ui/checkout/provinces?${queryParams.toString()}`, {
    method: 'GET',
  })) as Province[];
...
export async function getCities(provinceId: number, langCode?: string): Promise<City[]> {
  ...
  const response = (await apiRequest(`/api/ui/checkout/cities?${queryParams.toString()}`, {
    method: 'GET',
  })) as City[];
...
export async function saveAddress(
  ...
  const response = (await apiRequest(`/api/ui/checkout/address?${queryParams.toString()}`, {
    method: 'POST',
 ...
 * Set default address
 */
export async function setDefaultAddress(addressId: number, langCode?: string): Promise<void> {
  const queryParams = new URLSearchParams();
  if (langCode) queryParams.append('langCode', langCode);

  await apiRequest(`/api/ui/checkout/address/${addressId}/set-default?${queryParams.toString()}`, {
    method: 'POST',
  });
}
 ...
 * Delete an address
 */
export async function deleteAddress(addressId: number, langCode?: string): Promise<void> {
  const queryParams = new URLSearchParams();
  if (langCode) queryParams.append('langCode', langCode);

  await apiRequest(`/api/ui/checkout/address/${addressId}?${queryParams.toString()}`, {
    method: 'DELETE',
  });
}
...
export async function getDeliveryMethods(langCode?: string): Promise<CheckoutDeliveryMethod[]> {
  const queryParams = new URLSearchParams();
  if (langCode) queryParams.append('langCode', langCode);

  const response = (await apiRequest(
    `/api/ui/checkout/delivery-methods?${queryParams.toString()}`,
    {
      method: 'GET',
    },
